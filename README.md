...
Each participant saves one commander for every other participant. Their own two cards came in with their sign-up and are shown read-only — changing one is an organiser job rather than a self-service one, because they are already in the pool everyone else is drawing against. The app excludes each recipient's vetoed colour on both the search endpoint and the save action. When every required slot is filled, the submissions lock automatically. Duplicate recommendations are allowed while choosing, but the exchange does not unlock until every participant's pool holds at least four unique eligible cards.

Once unlocked, each deck builder receives a stable random set of four cards drawn from their recipient's two sign-up picks plus a recommendation from every other participant — the deck builder's own included, so a pool is `n + 1` cards for `n` people. Excluding the builder's own card was the earlier rule; it protected only the fact that a builder might recognise their own suggestion, never who contributed any of the others, and it cost a candidate from a pool that has little slack to spare. Three cards are shown. The fourth remains server-side and hidden until the participant permanently trades one visible card for it; that cash-in can only succeed once.
...
- **Commander Browser & Filtering:**
  ...
  - The client requests random batches from `/api/commanders/sample`, which enforces server-side veto exclusions and applies `cache-control: no-store` to guarantee fresh random samples on each roll.
  - **The batch size follows the breakpoint**, because the grid is `grid-cols-2 sm:grid-cols-3`: nine is three clean rows at three columns but four rows plus a single orphan at two, which reads as a gap rather than a layout. So a phone asks for **ten** (five rows of two) and anything wider asks for **nine**. Read when the request is built rather than held in state — as a dependency of `load` it would re-roll the whole grid on rotation, throwing away the cards somebody was reading to close a one-card gap. The re-roll button says "Roll again" rather than naming a number, for the same reason a hardcoded "nine" was wrong the moment this landed.
...
- **Scryfall Queries & Caching:**
  - Upstream queries:
    - Pool: `is:commander r:u game:paper -is:unset -e:slz` (742 cards as of September 2026; it moves with each set). Three filters change what is legal and are easy to lose in a reword, so a test pins all three. `game:paper`: without it the pool gains 49 Arena-only Alchemy cards — the "A-" rebalances and Alchemy Horizons — which cannot be bought, in an exchange where somebody hands over a physical deck. `-is:unset`: un-set cards are not commanders for this event. `-e:slz`: a set whose uncommon printings are not meant to affect legality here. Scryfall filters **printings**, not cards, so the exclusion drops only commanders whose *sole* uncommon paper printing is in that set — six of them — while Kazuul and Yargle stay because they are printed uncommon elsewhere too.
    - **No `f:edh`**, deliberately: format legality lags a set's release by weeks, so a spoiled set is invisible exactly while the group is keenest on it. Legality here is the event's own judgement — the type line, the rarity and this repo's ban list. Reality Fracture added 28 commanders this way. It lets in nothing banned in Commander (no banned card is an uncommon legendary creature), and costs seven Unfinity commanders that `-is:unset` removes.
    ...
    - Partner-capable: the pool query plus `otag:pair-commander` (~65 cards). Catches Partner, Partner with, "Choose a Background", and Backgrounds.
